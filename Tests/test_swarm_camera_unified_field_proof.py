...
def test_fresh_frame_and_visual_prove_camera_even_without_kernel_heartbeat(tmp_path: Path) -> None:
    now = 1000.0
    state = tmp_path
    _write_jsonl(
        state / "active_eye_identity_frames.jsonl",
        [{"ts": now - 2, "event": "ACTIVE_EYE_IDENTITY_FRAME", "device": "MacBook Pro Camera", "w": 1552, "h": 1552, "sha8": "abc12345"}],
    )
    _write_jsonl(
        state / "visual_stigmergy.jsonl",
        [{"ts": now - 1, "sha8": "abc12345", "w": 1552, "h": 1552}],
    )

    proof = build_camera_unified_field_proof(state, now=now)

    assert proof.status == "CAMERA_HEALTHY_NO_FACE_PROOF"
    assert proof.ok is True
    assert proof.camera_healthy is True
    assert proof.vision_health is None
